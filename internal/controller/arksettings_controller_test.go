...
var _ = Describe("ArkSettings Controller", func() {
	const (
		resourceName = "test-arksettings"
		namespace    = "default"
	)
	...
	Context("When reconciling a minimal ArkSettings", func() {
		BeforeEach(func() {
			By("creating an ArkSettings with no spec fields set")
			resource := &arkonisv1alpha1.ArkSettings{
		...
		It("should set Ready=True with reason Accepted", func() {
			...
			r := &ArkSettingsReconciler{Client: k8sClient, Scheme: k8sClient.Scheme()}
	...
	Context("When reconciling an ArkSettings with spec values", func() {
		BeforeEach(func() {
			By("creating an ArkSettings with temperature, outputFormat, and prompt fragments")
			resource := &arkonisv1alpha1.ArkSettings{
				ObjectMeta: metav1.ObjectMeta{
					Name:      resourceName,
					Namespace: namespace,
				},
				Spec: arkonisv1alpha1.ArkSettingsSpec{
					Temperature:   "0.7",
	...
	Context("When reconciling a nonexistent ArkSettings", func() {
