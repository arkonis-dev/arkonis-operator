...
var _ = Describe("ArkMemory Controller", func() {
	const (
		namespace = "default"
	)
	ctx := context.Background()

	newReconciler := func() *ArkMemoryReconciler {
		return &ArkMemoryReconciler{Client: k8sClient, Scheme: k8sClient.Scheme()}
	}

	reconcileAndFetch := func(name string) *arkonisv1alpha1.ArkMemory {
	...
	Context("when the ArkMemory does not exist", func() {
